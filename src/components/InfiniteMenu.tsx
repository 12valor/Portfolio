...
// NEW: Uniform to blend grayscale and color
uniform float uMoveAlpha;
...
void main() {
    ...
    // Ensure image aspect is correctly handled
    ...
    // --- BLACK & WHITE AT FIRST LOGIC ---
    // Convert texture color to grayscale using standard luminosity weights
    float gray = dot(texColor.rgb, vec3(0.299, 0.587, 0.114));
    vec3 grayColor = vec3(gray);

    // NEW: Blend grayscale with original color based on interaction (uMoveAlpha)
    vec3 finalRgb = mix(grayColor, texColor.rgb, uMoveAlpha);
...
class ArcballControl {
  ...
  constructor(canvas: HTMLCanvasElement, updateCallback?: UpdateCallback) {
    this.gl = canvas.getContext('webgl2', { 
        antialias: true, 
        alpha: true,              // <--- Allows the background to be see-through
        premultipliedAlpha: false // Prevents dark edges on transparent elements
    })!;
    this.canvas = canvas;
...
class InfiniteGridMenu {
  ...
  private discLocations!: {
    ...
    uMoveAlpha: WebGLUniformLocation | null;
  };
